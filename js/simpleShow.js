...
function simpleShow(){	
	...
	function imgCss(){
		$("#simpleShow img").css({'max-height': + ($(window).height()-30) + 'px','max-width': + ($(window).width()-30) + 'px'});};
	...
	$("a.viewThumb").click(function(){
		...
		$("#simpleShow").fadeIn(100, function(){$("#sSCenter").html("<img src='"+href+"'></div><div id='simpleClose'>x</div>");
		imgCss();
	});
	//make it go away	
